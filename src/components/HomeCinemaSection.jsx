...
const HomeCinemaSection = ({ title, showVideoPromo, videoAdId }) => {
  ...
  const cinemaProducts = useMemo(() => {
    if (!products) return [];
    return products.filter(p => p.homeCinemaPlacement === true);
  }, [products]);

  // Only show if there are products
  if (cinemaProducts.length === 0) return null;
      ...
      <SectionLayoutWrapper showVideoPromo={showVideoPromo} videoAdId={videoAdId}>
        ...
        <div className="flex flex-col items-start text-left mb-10">
          <div className="flex flex-col items-start">
            ...
            <p className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-[0.3em]">Premium Audio & Visual Experience</p>
          </div>
        </div>
...
export default HomeCinemaSection;
