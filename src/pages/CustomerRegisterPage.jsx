...
const CustomerRegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        if (logUserActivity) {
          logUserActivity(result.user?.id, result.user?.display_name || name, 'registration');
        }
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="pt-4">
              <button
                type="submit"
                disabled={isLoading}
                className="group relative w-full flex justify-center py-4 px-4 border border-transparent text-sm font-bold uppercase tracking-widest rounded-2xl text-white bg-[var(--primary-color)] hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-[var(--primary-color)] disabled:opacity-50 transition-all"
              >
                {isLoading ? (
